...
启动 `serve` 后，打开终端显示的管理面板，用同一终端的管理口令进入。到“订阅与代理”选来源，填地址或文件路径，测试通过后点应用。已经开着代理软件的，通常直接填本地 HTTP / SOCKS5 地址最省事。

这里有两个不同的测试：

- **来源测试：** 读取或下载、解析、构建出站。不会发送模型请求，也不证明节点能连通。
- **路由连接测试：** 通过指定出口给所选上游发不带登录信息的请求。收到 HTTP 响应只说明网络有回应，不证明能采到 292。
